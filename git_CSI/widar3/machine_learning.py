...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input_seq):
        ...
        global_feat = torch.mean(gru_out, dim=1)
        logits = self.decoder(global_feat)
        return logits

def crnn_test():
    save_model_path = "./CRNN_6Class_BiGRU_Norm/"
    ...
    test_size = full_len - train_size # 修复：总长度减去训练集，保证相加相等
    ...
    rnn = DecoderRNN(class_num).to(device)
    ...
    for epoch in range(epoch_total):
        ...
        if val_acc > best_acc:
            ...
            torch.save(rnn.state_dict(), os.path.join(save_model_path, "best_rnn.pth"))
