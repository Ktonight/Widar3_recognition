...
class Bfee:
    ...
    def from_file_intel(filename, model_name_encode="shift-JIS"):
        ...
        while cur < (bfee.file_len - 3):
            ...
            if bfee.code == 187:
                ...
                bfee.csi = np.zeros(shape=(30, bfee.Nrx, bfee.Ntx), dtype=np.dtype(complex))  # csi初始化
                bfee.perm = [1, 2, 3]
                ...
                if sum(bfee.perm) != triangle[bfee.Nrx - 1]:
                    ...
                    temp_csi = np.zeros(bfee.csi.shape, dtype=np.dtype(complex))
    ...
    def fill_csi_matrix(array, cur, Nrx, Ntx, num_tones):
        csi = np.zeros(shape=(num_tones, Nrx, Ntx), dtype=np.dtype(complex))  # csi初始化  ~~
